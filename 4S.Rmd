...
# use distance formulae to estimate parameters under the K80 model
...
$$\hat{\kappa} = \frac{2\ln(1-2S-V)}{\ln(1-2V)}-1 = `r round(kappa_hat_K80, digits = 3)`$$ 
...
rate_matrix <-
  frame_matrix(
    ~T, ~C, ~A, ~G,
    0-kappa_hat_K80*pi_C-pi_A-pi_G, kappa_hat_K80*pi_C, pi_A, pi_G,
    kappa_hat_K80*pi_T, 0-kappa_hat_K80*pi_T-pi_A-pi_G, pi_A, pi_G,
    pi_T, pi_C, 0-pi_T-pi_C-kappa_hat_K80*pi_G, kappa_hat_K80*pi_G,
    pi_T, pi_C, kappa_hat_K80*pi_A, 0-pi_T-pi_C-kappa_hat_K80*pi_A
  )
```
...
log_likelihood_total <- log_likelihood(v, kappa_hat_K80, avg_frequencies, mt_cyb_df)
```
...
simulations_seqs <-
  ...
  nest(seqs = -alignment) %>%
  mutate(l = log_likelihood_vectorized_df(v, kappa_hat_K80, avg_frequencies, seqs))
```
...
ml_two_parameter <- optim(c("v" = v, "kappa" = kappa_hat_K80), neg_likelihood_two_parameter)
```
