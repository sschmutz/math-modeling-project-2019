...
To simulate two aligned sequences over a distance of $v = 0.3$ and the previously defined substitution rate matrix $Q$ 
it's assumed that the substitutions happen independantly across sites.  
1000 pairwise alignments of length 100 will be simulated.

```{r simulation, echo=FALSE}
set.seed(202001)

transition_matrix_03 <- expm(rate_matrix*0.3)

simulations <-
  crossing(alignment = 1:10,
           site = 1:10) %>%
  rowwise() %>%
  ...
  rowwise() %>%
  mutate(human = sample(nt_order, size = 1, prob = transition_matrix_03[mouse_index,])) %>%
  ungroup() %>%
...
simulations_seqs <-
  ...
  nest(seqs = -alignment) %>%
  mutate(l = log_likelihood_vectorized_df(0.3, kappa_hat, avg_frequencies, seqs))
