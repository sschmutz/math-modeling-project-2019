...
The estimate of transition transversion rate ratio ($\kappa$) can be calculated as follows [@doi:10.1093/acprof:oso/9780199602605.001.0001]:  
$$\kappa = \frac{2\ln(1-2S-V)}{\ln(1-2V)}-1 = `r round(kappa_hat, digits = 4)`$$ 
...
```{r rate-matrix, echo=FALSE, results='asis'}
...
from human and mouse is $d = 0.01$. This means that the predicted actual 
mutations between the two sequences is 0.01 per position.  
We assume that the mouse sequence is ancestral to the human sequence.  
Using the following formulae [@wiki:Models_of_DNA_evolution], the the transition probabilities can be calculated:

$$
\lambda = \frac{1}{2(\pi_{A}+\pi_{G})(\pi_{C}+\pi_{T})+2\kappa(\pi_{A}\pi_{G}+\pi_{C}\pi_{T})}
$$

$$
P_{AA}(d,\kappa,\pi) = \frac{\pi_{A}(\pi_{A}+\pi_{G}+(\pi_{C}+\pi_{T})e^{-\lambda d})+\pi_{G}e^{-(1+(\pi_{A}+\pi_{G})(\kappa-1))\lambda d}}{\pi_{A}+\pi_{G}}
$$
