...
log_likelihood_vectorized <- Vectorize(log_likelihood, vectorize.args = "kappa")

ggplot(data.frame(kappa = c(0, 7)), aes(kappa)) +
  stat_function(fun = log_likelihood_vectorized, args = list(v = 0.01, pi = avg_frequencies, df = mt_cyb_df)) +
  xlim(0, 6) +
  ylim(-3150, -3100) +
  theme_minimal() +
