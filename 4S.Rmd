...
likelihood_df <- tibble(kappa = seq(0.5, 10, by = 0.1))
l <- c()

for (kappa in likelihood_df$kappa){
  l <- c(l, log_likelihood(0.01, kappa,avg_frequencies, mt_cyb_df))
}

likelihood_df <- bind_cols(likelihood_df, l = l)

ggplot(likelihood_df, aes(x = kappa, y = l)) +
  geom_line() +
  xlim(0, 6) +
  ylim(-3150, -3100) +
  theme_minimal() +
